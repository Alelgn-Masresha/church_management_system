...
import { Input, SectionHeader, RadioGroup, Select, BilingualLabel } from '../FormControls';

interface Props {
  data: FormData;
  update: (fields: Partial<FormData>) => void;
  errors?: Record<string, string>;
  show?: 'testimony' | 'ministry' | 'both';
}

export const ReligiousAndMinistry: React.FC<Props> = ({ data, update, errors = {}, show = 'both' }) => {
        ...
        <section>
          ...
          <div className="mb-6">
            <RadioGroup
              ...
              required
              error={errors.religiousBackground}
          ...
          <div className="bg-white p-6 border border-gray-200 rounded-lg space-y-4 shadow-sm">
            ...
            <div className="pt-4">
              <BilingualLabel
                ...
                amharicLabel="ወደዚህ ቤተክርስቲያን እንዴት መጡ?"
                required
                error={errors.howDidYouCome}
              />
              <textarea
                className={`w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 ${errors.howDidYouCome ? 'border-red-500 focus:ring-red-200' : 'border-gray-300 focus:ring-blue-500'
                  }`}
                rows={2}
                value={data.howDidYouCome}
                onChange={e => update({ howDidYouCome: e.target.value })}
              ></textarea>
              {errors.howDidYouCome && <p className="text-[10px] text-red-600 font-bold uppercase mt-1">{errors.howDidYouCome}</p>}
            </div>
          </div>
        </section>
      )}
